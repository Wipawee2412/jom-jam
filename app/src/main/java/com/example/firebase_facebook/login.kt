        ...
        button.setOnClickListener {
            ...
            if(msgUsrInput.trim().length>0&&passUsrInput.trim().length>0){
                if(msgUsrInput!=passUsrInput){
                    ...
                    if(msgUsrInput=="60160351"){
                        Toast.makeText(context,"Wellcome : "+usrInput.text.toString()+" !!! ", Toast.LENGTH_LONG).show()

                        val fragment_seven_wonderworld = seven_wonderworld()
                        val fm = fragmentManager
                        val transaction : FragmentTransaction = fm!!.beginTransaction()
                        transaction.replace(R.id.layout, fragment_seven_wonderworld,"fragment_seven_wonderworld")
                        transaction.addToBackStack("fragment_seven_wonderworld")
                        transaction.commit()
                    }else{
